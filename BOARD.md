...
| ROTA-T054 | Owner/Architect | CC/Codex | `task/ROTA-T054-persisted-plan-preview-contract` | `163e30c3a6f0e6ed49350f06c9f2ebd4e3ee78cc` | CODEX_REPORTED | **OWNER DECISION: R5-01 ODRZUCONY jako rozszerzenie kontraktu / edge-case bez realnego wymogu produktowego.** Nie dodawać żadnego nowego trwałego markera, kolumny, tabeli, statusu ani recovery tylko po to, aby po reloadzie pamiętać, że wcześniejszy cleanup preview nie powiódł się przy transientnym `TECHNICAL_ERROR`/`SEARCH_INCOMPLETE`/`NARROW_SEARCH_EXHAUSTED`/`NO_ALTERNATIVE`. T54-06 wymaga widocznego błędu persistence i zachowania świeżego wyniku w bieżącej sesji; ten przypadek ma PASS. Dla `DECISION_REQUIRED` istniejący trwały readback jest używany i również ma PASS. R5-01 nie blokuje T054. Nie poprawiać kodu pod ten finding. |
