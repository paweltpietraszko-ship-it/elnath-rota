...
|---|---|---|---|---|---|---|
| ROTA-E2E-AUDIT-R1-TESTS-STALE | CC | Architekt | — (finding, brak briefu) | `40d1e95` (stan sprawdzony) | OWNER_DECISION_NEEDED | **Znalezione przy okazji ROTA-TECHNICAL-ERROR-RECOVERY-UX, nie naprawione — poza zakresem tamtego Tasku.** `frontend/e2e/diagnostics.audit-r1.spec.ts` ma dwa testy failujące już na czystym, nietkniętym stanie sprzed tamtego Tasku (`40d1e95`) — zweryfikowane bezpośrednio przez tymczasowy worktree na tym dokładnym SHA, więc to nie regresja z żadnej mojej zmiany. (1) `"a SITE identifier is removed from recorded endpoint paths"` — SITE identifier nie jest już usuwany z zapisanych ścieżek endpointów w diagnostyce frontu (sprzeczne z tym, co test miał potwierdzać). (2) `"ordinary dev runtime has no clickable failure-injection controls"` — zwykły dev runtime pokazuje 5 klikalnych przycisków wstrzykiwania awarii (`[data-testid="diag-test-hooks"] button"`) zamiast oczekiwanego 0 — sugeruje, że `__E2E_TEST_HOOKS__` (flaga sterująca widocznością `TestHooks.tsx` w `App.tsx`) jest dziś zawsze prawdziwa albo nie jest już poprawnie gated do trybu e2e. CC nie diagnozował głębiej przyczyny ani nie projektował naprawy — to wymaga architekta: czy to prawdziwa regresja identyfikatorów/redakcji w diagnostyce frontu (potencjalnie RODO-istotna, T060-adjacent), czy tylko przestarzały test względem zmienionej architektury `TestHooks`/flagi e2e. |
