...
| ROTA-LOGIN-VISUAL-REDESIGN | CC | Codex | `task/ROTA-LOGIN-VISUAL-REDESIGN` | `9753227` | READY_FOR_CODEX | Owner zobaczył realny ekran logowania na Railway i ocenił go negatywnie wizualnie. Root cause: `Login.tsx` używał `var(--surface, #fff)`, a `--surface` nigdy nie było zdefiniowane w `App.css` -- zawsze spadało na sztywny biały box, kontrastujący z ciemnym motywem grafit/mosiądz reszty apki. Przepisane na realne tokeny/komponenty apki (`--paper-raised`, `--brass`, ten sam znak marki co topbar Workspace, `.create-panel-fields`). Zero zmian logiki/kontraktu -- czysto wizualne. Zweryfikowane: `tsc -b` czyste, zbudowany frontend obejrzany na żywo w przeglądarce (lokalny serwer z `ROTA_CENTRAL_SERVICE=1`). |
