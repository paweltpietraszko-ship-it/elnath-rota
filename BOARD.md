...
| ROTA-EQUAL-SPLIT-FALLBACK-IGNORES-ABSENCE | Codex | Architekt | `task/ROTA-EQUAL-SPLIT-FALLBACK-IGNORES-ABSENCE` | precheck `607eada` of brief `8176f50` (base `c4fad96`) | CODEX_REPORTED | **WYMAGA KOREKTY PREIMPLEMENTATION R1.** Zachowanie OWNERA i minimalne szwy są zasadniczo poprawne, ale brief nie zamraża jednego testowalnego kontraktu odpowiedzi dla blockera brakujących celów (precheck/PLAN/cała rodzina REPLAN), a `precheck.py` pozostaje w TASK_SCOPE bez określonej roli. Brakuje też wymaganego `WHERE_MAP: REQUIRED` dla usuwanego helpera/call site, nowego gate i zmienianych endpointów. Zamknięta lista: `tasks/ROTA-EQUAL-SPLIT-FALLBACK-IGNORES-ABSENCE/round_01/tests/tests_r1.txt`. Bez testów i audytu diffu produktu. |
| ROTA-DELEGACJA-ONLY-EMPLOYEE-MISSING-FROM-SCHEDULE-GRID | Codex | Architekt | `task/ROTA-DELEGACJA-ONLY-EMPLOYEE-MISSING-FROM-SCHEDULE-GRID` | precheck `34092e0` of brief `8b9c748` (base `c4fad96`) | CODEX_REPORTED | **WYMAGA KOREKTY PREIMPLEMENTATION R1.** Kontrakt zachowania, minimalny helper projekcji i reuse istniejących ownerów są kompletne; jedyny brak to wymagany `WHERE_MAP: REQUIRED` dla zmienianych DTO/helperów/endpointu i `ScheduleGrid`. Po jego dodaniu implementacja nie wymaga nowej decyzji produktowej. Raport: `tasks/ROTA-DELEGACJA-ONLY-EMPLOYEE-MISSING-FROM-SCHEDULE-GRID/round_01/tests/tests_r1.txt`. Bez testów i audytu diffu produktu. |
