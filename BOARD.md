...
| ROTA-ASSEMBLER-STALE-EQUAL-SPLIT-WARNING-TEXT | CC | Codex | `task/ROTA-ASSEMBLER-STALE-EQUAL-SPLIT-WARNING-TEXT` | impl `b5163fc` (base `4c94de0`) | READY_FOR_CODEX | **Zaimplementowane wg OWNER_ACCEPTED 2026-09-15.** `rota/application/assembler.py::_assemble_work_balances`: drugie zdanie ostrzeżenia zmienione z „użyto awaryjnego, równego podziału godzin" (opisywało usunięty już mechanizm) na „ustaw godziny docelowe (Karta pracownika)"; samo ostrzeżenie zostaje. Zaktualizowana bezpośrednia asercja w `tests/test_t009_open_and_assembler.py::test_5_missing_target_hours_not_invented_and_demand_count_unchanged` (jedyny żywy konsument tego tekstu — zarchiwizowany `tasks/ROTA-UI-TARGET-HOURS-IDENTIFIER/round_01/tests/test_audit_r1.py` nie jest zbierany przez pytest, `testpaths=["tests"]`, celowo nietknięty jako historyczny artefakt audytu). Zero zmian gate/solvera/zachowania — czysto tekstowe. Zweryfikowane: scoped run (10 plików dotykających assembler.py) = 3 znane pre-existing failures (potwierdzone wcześniej na czystym `main`), 0 nowych regresji; ruff czyste. |
