...
| ROTA-T065 | Codex | CC | `task/ROTA-T065` | poprawka `4274011`; audit R4 FAIL + repro `56e52bf` | CODEX_REPORTED | **New ORDINARY R3-01 naprawione, ale legacy ORDINARY cicho traci ograniczenia D/N.** Obowiązujące repro R2+R3: 7 passed, 1 OWNER-deselected. Celowany zestaw: 214 passed, 1 deselected. R4-01: `dn_semantics_apply()` zwraca false dla każdego ORDINARY, więc nie odróżnia new od legacy wymaganych przez §8/§11/§16. Dwa niezależne repro R4 są czerwone: legacy N (`shift_kind=None`, klasyfikowane z katalogu) z HARD SiteRule „tylko D” kończy PLAN jako FEASIBLE; analogiczne okno EXTERNAL „tylko D” także dopuszcza N. To jest zakazane w §11 ciche rozszerzenie, nie kwestia etykiety ani nowa decyzja produktowa. Raport: `tasks/ROTA-T065/round_01/tests/tests_r4.txt`. Pełnej regresji nie uruchomiono: brak jawnej zgody OWNERA i dokładny reproduktor już rozstrzyga wąski re-check; symulatory i benchmarki pozostają zamrożone. Poprawka musi rozróżnić new ORDINARY od legacy ORDINARY; new nie konsumuje D/N, legacy nie może zostać potraktowane jako nieograniczone — ma wejść w jawny istniejący flow korekty. Następny re-check wyłącznie diff `4274011..NOWY_SHA`, repro R4, obowiązujące R2/R3 i celowane testy. |
