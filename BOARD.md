...
| ROTA-PRINT-IGNORES-UNACKED-DEVIATIONS | OWNER | Architekt | — (pre-brief finding, zależny od czytelnych deviations) | main@`48c19bf` | OWNER_DECISION_NEEDED | **OWNER_CORRECTED 2026-09-05: jedno miejsce potwierdzania odchyleń, nie dwa.** Poprzedni wpis (Codex, "zgoda na LAW jest jednorazowa, niezależna od Finalizuj") zakładał DRUGI, osobny mechanizm zgody obok istniejącego "Finalizuj" — OWNER to odrzuca: koordynator ma dokładnie jedno miejsce, gdzie potwierdza odchylenia, nie dwa równoległe. Architekt ma przeprojektować mechanizm tak, żeby: (a) blokada eksportu na niepotwierdzonym LAW i (b) wymóg finalizacji korzystały z tego samego, jedynego potwierdzenia — nie z osobnego, równoległego flow tylko dla eksportu. Pozostałe ustalenia z audytu (blokują wyłącznie LAW, inne kategorie nie blokują, techniczne granice egzekwowane przez backend) zostają w mocy — zmienia się tylko to, że nie wolno dokładać drugiego miejsca potwierdzania. Pełny audyt: `arch/PREBRIEF_AUDIT_2026-09-05_T056_FOLLOWUPS.md`. |
