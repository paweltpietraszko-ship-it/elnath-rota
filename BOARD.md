...
| ROTA-T065-PRINT-GAP | Architekt | Codex | `task/ROTA-T065-PRINT-GAP` | brief `f27d890e2a0acc3551fc82eff862dc48560e5afa` | READY_FOR_CODEX | **WĄSKI PREIMPLEMENTATION REVIEW. Produkcja nadal HOLD.** Sprawdź dokładny `tasks/ROTA-T065-PRINT-GAP/brief.md`: jeden `generate_schedule_pdf`/endpoint/lifecycle; wspólne LAW acknowledgement, CURRENT race checks, lineage/provenance/revision; rozgałęzienie tylko prezentacyjne po `planning_regime`; OCHRONA bez zmiany kontraktu; ORDINARY z realnych Assignment start/end i historycznego `ShiftDemand.required_role`, bez D/N i bez własnego bilansu; jeden regime-aware `SitePrintSettings`, bez drugiego ownera. Sprawdź szczególnie wiele legalnych work-items jednego dnia, historyczny reprint i izolację CHECKPOINT A. Brief używa parser-compatible literalnego `TASK_SCOPE:`. PASS nie odblokowuje produkcji: po audycie CC ma wykonać wyłącznie syntetyczną, izolowaną próbkę PDF pod `tasks/ROTA-T065-PRINT-GAP/prototype/**`; OWNER musi zaakceptować wygląd przed CHECKPOINT B. |
