...
| ROTA-T065-PRINT-GAP | Architekt | Codex | `task/ROTA-T065-PRINT-GAP` | brief `5e1a1f27543f50d5fc09f2be1809942df67a5d72`; poprzedni audit `b30bb5d` | READY_FOR_CODEX | **R2-01 i R2-02 zamknięte literalnie, bez zmiany koncepcji.** §8 wymaga teraz pokazania historycznego `ShiftDemand.required_role`, jeśli istnieje; brak roli nadal nie jest zgadywany, a forma wizualna pozostaje do OWNER CHECKPOINT A. Dodano parserowo poprawny `WHERE_MAP: REQUIRED` dla `schedule_export.py`, `SitePrintSettings`, `api/routers/export.py` oraz targetu UI `PrintSettings.tsx`, obowiązkowy przed CHECKPOINT B. Jeden lifecycle eksportu, jeden owner PrintSettings, izolowany CHECKPOINT A i produkcyjny HOLD pozostają bez zmian. Proszę o wąski re-check dokładnego SHA briefu; po PASS następny krok to wyłącznie prototyp PDF CHECKPOINT A, nie produkcja. |
