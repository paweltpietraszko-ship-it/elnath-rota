...
| AGENT-HARNESS-KIT-AUDIT | CC | Codex | Agent: `task/HARNESS-STARTER-KIT` | `84676290fcf3f6aea28cb89edde784a79faa08be` | READY_FOR_CODEX | **RE-AUDYT r2 po FAIL r1** (raport r1: `tasks/AGENT-HARNESS-KIT-AUDIT/audit_r1.txt`). Repo Agent, nie Rota. Poprawki CC wg findingow: **F1** `BASE` w backend.py -- `before_sha` musi byc punktem odejscia od `origin/main`/`main` (`merge-base`), workflow liczy go sam i juz nie czyta `repo_before.hash`; **F2** `is_artifact` zwalnia spod zakresu tylko `.txt/.md/.hash` pod `tasks/` (kod tam podlega zakresowi i limitom; `log/` usuniete); **F3** brief czytany z blobu `head_sha`, nie z working tree; **F4** hook czyta nazwy `-z` przez `IFS= read -r`, blokuje tez nazwe z nowa linia. WYMAGA_DECYZJI: symlinki = domyslnie FAIL (`SYMLINK`), `**` bez zmiany zachowania (jeden lub wiecej poziomow), opisane w `docs/PROCESS.md` -- do potwierdzenia przez OWNERA. Nowe reproduktory w testach (68 zielone u CC, kazda poprawka potwierdzona mutacja). **Zakres re-audytu (wg Twojego WARUNKU):** oryginalne reproduktory findingow 1-4, celowane testy zmienionych funkcji (`check_base`, `check_symlinks`, `is_artifact`, `read_brief`, `run_backend`, hook, `gate.yml`), bez pelnej macierzy. Zwroc uwage: czy `check_base` ma obejscie (np. wiele refow, `main` lokalny wyprzedzony, merge main do brancha) i czy wnioski z lokalnego `main` sa bezpieczne. Raport: `tasks/AGENT-HARNESS-KIT-AUDIT/audit_r2.txt` na tym samym branchu `audit/agent-harness-kit`. |
