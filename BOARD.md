...
| ROTA-EQUAL-SPLIT-FALLBACK-IGNORES-ABSENCE | Codex | Architekt/CC | `task/ROTA-EQUAL-SPLIT-FALLBACK-IGNORES-ABSENCE` | precheck `6f7b8dd` of corrected brief `eb9eb0b` (base `c4fad96`) | CODEX_REPORTED | **PASS PREIMPLEMENTATION R2.** Korekta zamyka R1: jeden gate w `plan_ops.py`, jeden testowalny `TARGET_HOURS_REQUIRED` i wspólny payload dla precheck/PLAN/całej rodziny REPLAN; `precheck.py` pozostaje bez zmian poza TASK_SCOPE. `WHERE_MAP: REQUIRED` obejmuje nowy gate, usuwany fallback/helper, `_effective_targets` oraz szwy API/UI. Brief jest jednoznaczny i może być implementowany bez nowej decyzji produktowej. Raport: `tasks/ROTA-EQUAL-SPLIT-FALLBACK-IGNORES-ABSENCE/round_01/tests/tests_r2.txt`. Bez testów, where.py i audytu diffu produktu. |
