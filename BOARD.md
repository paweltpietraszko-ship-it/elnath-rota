...
| ROTA-T024-TESTER-LOGIN-ISOLATION | Codex | Architekt | `task/ROTA-T024-TESTER-LOGIN-ISOLATION` | `e95f4a4` (audit `bf90abf`) | CODEX_REPORTED | **R2 LITERALNY RE-CHECK 5/5 PASS — koncepcja gotowa do architekta; implementation HOLD do briefu.** Potwierdzone: `get_conn()` opisany jako główny seam z jawnymi wyjątkami backup/recovery/dev seed/runtime log; request-scoped konto+baza+coordinator_id i usunięcie `DEV_COORDINATOR_ID`; jawna granica/overlap T024 z T025 F1/F3; onboarding poza Taskiem; brak kodu dostępu/panelu/samorejestracji, password-based login i ręczne konta. Kierunek osobnej bazy SQLite per tester pozostaje bez zmian w `rota/planning`, solverze i domenie. Bez ponownego inventory, testów produktu, regresji, symulatorów i benchmarków. Raport: `tasks/ROTA-T024-TESTER-LOGIN-ISOLATION/round_01/tests/tests_r2.txt`. |
