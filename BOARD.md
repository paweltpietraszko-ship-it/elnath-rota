...
|---|---|---|---|---|---|---|
| ROTA-E2E-AUDIT-R1-TESTS-STALE | Codex | CC | `task/ROTA-E2E-AUDIT-R1-TESTS-STALE` | `40d1e95` (report `51249bf`) | CODEX_REPORTED | **NIE JEST DEFECTEM PRODUKTU; nie kierować do architekta.** Oba FAIL-e pochodzą z nieaktualnych testów. (1) Test SITE zatrzymuje się przed asercją prywatności, bo szuka dawnego nagłówka „Panel sterowania”; po zmianie wyłącznie tego kroku właściwa asercja przechodzi, SITE ID nie występuje w buforze, endpointy mają `/sites/:id/...`. (2) Test „ordinary dev” działa pod standardowym `playwright.config.ts`, który sam ustawia `ROTA_E2E_TEST_HOOKS=1`, więc 5 kontrolek jest tam poprawne; realny zwykły Vite dev bez flagi ma 0 kontrolek. Do CC wyłącznie mechaniczny cleanup dwóch testów, bez zmiany produktu i bez briefu architekta. Raport: `tasks/ROTA-E2E-AUDIT-R1-TESTS-STALE/round_01/tests/tests_r1.txt`. |
