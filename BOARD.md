...
|---|---|---|---|---|---|---|
| ROTA-BACKEND-TASK-SCOPE-FORMAT-DRIFT | Codex | Architekt/CC | `task/ROTA-BACKEND-TASK-SCOPE-FORMAT-DRIFT` | review `42e6c8e` na draft `49285a7` (brief/tests `109d02f`) | CODEX_REPORTED | **PREIMPLEMENTATION: jedna korekta testów przed implementacją.** Kontrakt poprawnie zamraża trzy decyzje OWNERA i zachowuje istniejących ownerów; nie wymaga nowej decyzji produktowej. Baseline odtworzony: 26 PASS / 21 FAIL. Brakujący dowód: `test_unreadable_brief` sprawdza tylko brak pliku; brief z niepoprawnym UTF-8 nadal daje surowy `UnicodeDecodeError` zamiast kontrolowanego `STATUS: FAIL` z powodem `TASK_SCOPE:` i bez szumu `DIFF_SCOPE:`. Dodać także pozytywną granicę, że scope kończy osobny nagłówek Markdown z dalszym tekstem. Zachować `audit_r2_repro.py`. Zredukować powtarzany iloczyn wariantów CLI do reprezentatywnych pionów, pozostawiając pełne klasy w testach bezpośrednich. Raport: `tasks/ROTA-BACKEND-TASK-SCOPE-FORMAT-DRIFT/round_01/tests/tests_r2.txt`. `backend.py` nadal HOLD; po korekcie wystarczy literalny re-check kontraktu/testów. |
