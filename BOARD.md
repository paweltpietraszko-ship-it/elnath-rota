...
| ROTA-T065 | Codex | CC | `task/ROTA-T065` | poprawka `2e4df35`; audit R3 FAIL + repro `eebed90` | CODEX_REPORTED | **Decyzja OWNERA przyjęta: ORDINARY jest ogólne, role są opcjonalne.** Reproduktor wymagający odrzucenia role-less ORDINARY został wyłączony i nie wpływa na werdykt. R2-01 PASS; DAY_ONLY/NIGHT-STREAK oraz katalog OCHRONA PASS. **Pozostaje R3-01:** `is_role_based_demand = required_role is not None` nie może być właścicielem rozróżnienia new ORDINARY po decyzji o opcjonalnych rolach. Ponadto SiteRule i external nadal stosują techniczne D/N do role-bearing new ORDINARY, mimo literalnego §8/§11. Niezależne repro R3: 3/3 failed — wynik identycznego demandu zależy od technicznego D/N przez SiteRule i external; role-less new ORDINARY z katalogu dziedziczy legacy D/N SiteRule i PLAN kończy się DECISION_REQUIRED. Raport: `tasks/ROTA-T065/round_01/tests/tests_r3.txt`. Testy Tasku i celowanych ownerów: 151 passed, 1 OWNER-deselected. Pełnej regresji, symulatorów i benchmarków nie uruchamiano. Dodatkowo niezmieniony `ControlPanel` nadal pokazuje role sklepowe w OCHRONA; nie jest to źródło FAIL, lecz do domknięcia przy tej samej informacji o reżimie. Następny re-check wyłącznie diff `2e4df35..NOWY_SHA`, obowiązujące repro R2 + 3 repro R3 i celowane testy. |
