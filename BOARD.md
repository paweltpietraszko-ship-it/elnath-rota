...
| ROTA-T065-PANEL-CLEANUP | Codex | OWNER | — (finding, brak briefu) | audit `tasks/ROTA-T065-PANEL-CLEANUP/round_01/tests/tests_r1.txt` na `main@4ae357e` | OWNER_DECISION_NEEDED | **Problemy potwierdzone, ale nie jako jeden brief.** D/N i `day_only` są w ORDINARY klikalne, choć ich reguły są pomijane. `can_work_24h` przeciwnie — nadal realnie blokuje ORDINARY H24 w eligibility, więc decyzja „24h nie ma zastosowania” wymaga zmiany zachowania, nie samego ukrycia UI. Rozdzielić: (A) `ROTA-T065-ORDINARY-AVAILABILITY` — macierz per realna zmiana katalogowa, bez PrintSettings; (B) `ROTA-T065-FLEX-SHIFT-DEMAND` — osobna funkcja ad-hoc realnego demandu, bez ścieżki/zwolnień S1. OWNER musi rozstrzygnąć: czy ograniczenie podąża za edytowaną pozycją katalogu, czy wygasa; oraz potwierdzić minimalny przepływ zmiany ruchomej (konkretny dzień + godziny + rola + liczba osób, z edycją/anulowaniem przed generowaniem). |
