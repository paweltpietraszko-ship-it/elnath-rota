...
- **"+ Dodaj osobę"**: picker over two paths, LOCAL only per the scope
  ...
  - *Nowy pracownik*: form (display_name, day_only checkbox) →
    ...
    R7-2) — all 5 constructor fields, only 2 are coordinator inputs:
    - `employee_id` — generated by the API layer (`uuid.uuid4().hex`),
      same pattern as Screen 1's `site_id`/`profile_id`.
...
- **Per-employee screen — "Zgłoś nieobecność"**: one form, all 5
  ...
  `active=False` chain-ends too, shown as past/ended entries, not
  filtered out.
