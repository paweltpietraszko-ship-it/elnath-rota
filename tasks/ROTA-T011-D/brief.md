...
`tests/test_t011_d_quarter_balance.py` są jedynymi dwoma nowymi plikami —
dokładnie limit `MAX_NEW_FILES = 2`. `tests/test_t009_open_and_assembler.py`
jest istniejącym plikiem, dopisanym do scope po FINDING D-R3-1
(`tasks/ROTA-T011-D/round_01/tests/tests_r3.txt`): jego istniejąca asercja
łącznej liczby ostrzeżeń stała się nieaktualna wskutek zachowania już
wymaganego przez CONSTRAINT pkt 5 tego briefu (jeden dodatkowy, kontraktowy
warunek ostrzeżenia o carry-in dla pracownika, który ma normę planowanego
miesiąca, ale nie ma jej we wcześniejszym miesiącu kwartału). Dozwolona
zmiana w tym pliku jest wąska: zastąpienie jednej starej asercji łącznej
liczby ostrzeżeń rozbiciem na `omitted_warnings == len(employees) - 1` i
`carry_in_warnings == 1`, bez tłumienia kontraktowego ostrzeżenia carry-in.
