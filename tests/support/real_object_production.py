...
def _availability(scenario: ScenarioSpec) -> tuple[AvailabilityRecord, ...]:
    ...
    for index, spec in enumerate(scenario.availability):
        ...
        records.append(AvailabilityRecord(
            record_id, f"{record_id}-v1", spec.employee_id, AvailabilityKind(spec.kind),
...
def _targets(scenario: ScenarioSpec) -> tuple[WorkBalance, ...]:
    ...
    return tuple(
        WorkBalance(employee, scenario.month, strict.get(employee, 144), 0, 0, 0, 0, 0)
        for employee in (*LOCAL_EMPLOYEES, *EXTERNAL_EMPLOYEES)
    )
...
def build_planning_state(scenario: ScenarioSpec) -> PlanningState:
    ...
    return PlanningState(
        ...
        boundary_assignments=tuple(
            _assignment(item, "benchmark-boundary") for item in scenario.boundary_assignments
        ),
        ...
        existing_assignments=tuple(
            _assignment(item, version_id) for item in scenario.fixed_demand_assignments
        ),
        deviations=(),
        work_balances=_targets(scenario),
        holiday_history=(),
        other_site_assignments=tuple(
            _assignment(item, "benchmark-other-site") for item in scenario.other_site_assignments
        ),
